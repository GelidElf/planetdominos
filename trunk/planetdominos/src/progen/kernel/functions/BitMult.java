...
public class BitMult extends Function{
	...
	public Object evaluate(List<Node> arguments, UserProgram userProgram, HashMap<String,Object> returnAddr){
		...
		for (int i = 0; i < 32; i++) {
			...
			int mask = 1 << i;
			//Make X AND mask. Then count the number of bits set to 1 in the result. There
			//will be one or zero bits set to 1. If there is one, that means that there is
			//a 1 in position x_i, or a 0 otherwise.
			int count = Integer.bitCount(mask & X);
			//If there were a 1 in x_i, then add to result the value 2^i * Y (or which is the same Y << i)
			if (count > 0) {
				result += Y << i;
			}
		}
		
		//System.out.println("DEBUG: Resultado esperado = " + X*Y + "; Resultado obtenido = " + result);
