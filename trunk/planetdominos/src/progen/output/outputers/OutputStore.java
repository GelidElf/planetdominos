...
/**
 * Clase que almacena todas las salidas disponibles en una ejecución concreta de ProGen.
 * Está implementado con un Singleton, de tal forma que únicamente existe una instancia
 * del almacén en toda la ejecución.
 * 
 * @author jirsis
 * @since 2.0
 * 
...
public class OutputStore {
	...
	/** Almacén que representa la única instancia del almacén a lo largo de la ejecución. */
	private static OutputStore store;

	/** 
	 * Constructor privado de la clase. Únicamente se ejecutará desde el método de crearción
	...
	/**
	 * Método de creación de la instancia única. En caso de existir una previamente, se devolverá
	 * una referencia a ésta.
	 ...
	 */
	public void init() {
		String outputersName[] = ProGenContext.getMandatoryProperty(
				"progen.outputers").split(",[ ]*");
		try {
	...
	/**
	 * Ejecuta el método correspondiente de cierre en todas las salidas disponibles.
	 */
	public void close() {
		for (Outputer out : outputers) {
			out.close();
