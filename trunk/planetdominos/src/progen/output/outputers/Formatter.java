...
/**
 * Clase que formatea distintas cadenas para que la justificación dentro de una cadena
 * sea centrada, a la izquierda o a la derecha.
 * @author jirsis
 * @since 2.0
 */
public class Formatter {
	 ...
	 * @param length El espacio donde centrarlo.
	...
	/**
	 * Justifica a la izquierda el texto pasado como parámetro, en la longitud solicitada.
	...
	public static String left(String text, int length) {
		...
		if(length>text.length()){
			stb.append(String.format("%" + (length - text.length()) + "s", " "));
		}
		return stb.toString();
	}

	/**
	 * Justifica a la derecha el texto pasado como parámetro, en la longitud solicitada.
	 * @param text El texto a justificar.
	 * @param length El espacio donde centrarlo.
	 * @return La cadena justificada en el espacio disponible.
	 */
	...
	/**
	 * Formatea un árbol proporcionado por parámetro de tal forma que devuelve un <code>String</code>
	 * en forma de programa Lisp, identado en varias líneas, según la profundidad del mismo.
	 * @param tree El árbol a formatear.
	...
	public static String tree(Tree tree) {
		...
		for(String token : tokens){
			function = false;
			if(token.charAt(0) == '('){
				//If the token is a function (starts with '(') we increment the indent and flag it as function. 
				indent++;
				function = true;
			}
			if(token.endsWith(")")){
				//If we are finalizing a function, we reduce the indent, we delete the last '\t' and we flat it
				//as no function. It is done in this way to work with functions "(function)"
				indent--;
				function = false;
	 ...
	 * @param indent El total de tabuladores a añadir.
