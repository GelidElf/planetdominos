 ...
 * 
 * Este operador selecciona un individuo en función del selector que tenga configurado
 * y selecciona de forma aleatoria un árbol de todos los que definen al individuo.
 * 
 * Una vez se tiene un árbol concreto, se selecciona un nodo de entre todos los posibles
 * y se busca colocar una función que sea compatible con la que esté en ese nodo
 * de entre todas las posibles que se puedan generar a partir de la gramática original.
 * 
 * Cuando una función es compatible con otra, se entiende una función tal que tenga
 * la misma signatura (número de argumentos necesarios) y mismo valor de retorno.
 * 
 * @author jirsis
 * @since 1.0
 */
public class PointMutation extends GenneticOperator {
	...
	/**
	 * Selecciona un nodo cualquiera del árbol pasado por parámetro y cambia 
	 * la función que está en ese nodo por otra función compatible, en función
	 * de las disponibles en la gramática que generó ese árbol.
	 * @param tree El árbol a mutar.
	 * @param grammar La gramática para obtener la nueva función.
	 */
	private void mutate(Tree tree, Grammar grammar) {
		boolean mutate=false;
		List<Integer> nodes = new ArrayList<Integer>();
		//se genera una lista aleatoria con los nodos a intentar mutar
		for(int i=0;i<tree.getRoot().getTotalNodes();i++) nodes.add(i);
		...
		while(!mutate && nodes.size()>0){
			 ...
			 */
			while (	(production.getFunction().compareTo(node.getFunction())==0
					|| production.getArgs().length != node.getFunction().getFunction().getArity())
					&& productions.size()>0) {
				
				production = productions.remove((int) (Math.random() * productions.size()));
			}
			
			//si no se han eliminado todas las producciones disponibles, se muta el nodo
