...
/**
 * Implementación del operador genético de cruce estándar.
 * El funcionamiento de este operador consiste en seleccionar dos individuos 
 * de la población pasada como parámetro, en función del selector que esté configurado.
 * 
 * Una vez seleccionados, se elige un nodo compatible de los dos árboles y se intercambian los 
 * dichos nodos y nodos los nodos hijos a estos.
 * 
 * Se entenderá como nodo compatible con otro, todo aquel que contenga una función que devuelva
 * el mismo tipo de valor de retorno, esto es así, para que cuando se intercambien los nodos,
 * los nodos resultantes sigan estando bien formados y sean palabras generadas por la gramática
 * original de cada árbol.
 * 
 * @author jirsis
 * @since 1.0
 */
public class StandardCrossover extends GenneticOperator {
	 ...
	 * @param treeA Primer árbol a cruzar.
	 * @param treeB Segundo árbol a cruzar.
	 * @return <code>true</code> si ha sido posible seleccionar dos nodos y
	 * cruzar los árboles, <code>false</code> en caso contrario.
	 */
	 ...
	 * los distintos tipos de operadores de árbol, para que funcione correctamente.
	 * @param treeA Uno de los árboles a cruzar.
	 * @param treeB El otro árbol a cruzar.
	 * @return Devuelve una lista, con dos elementos por lo general, que son 
	 * dos nodos compatibles entre ellos y en los que se puede aplicar el operador
	 * de cruce.
	 */
	protected List<Node> selectNodes(Tree treeA, Tree treeB){
		...
		crossNode1 = treeA.getNode(1 + (int) (Math.random()
				* treeA.getRoot().getTotalNodes() - 1));
		crossNode2 = treeB.getNode(1 + (int) (Math.random()
