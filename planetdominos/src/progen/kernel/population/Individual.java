 ...
 * 
 */
public class Individual implements Task, Comparable, Cloneable {
	/** Para serialización */
	...
	/**
	 * Representación del individuo para ser impresa de alguna forma.
	 * Modifica la versión estándar en la que se muestran todos los árboles (ADF y RPB)
	 ...
	 * @param variables Almacén que contiene todas las variables definidas por un identificador y
	 * su valor concreto.
	 * @param userprogram Definición del problema que ha tenido que implementar el usuario.
	 */
	private void evaluateTree(HashMap<String,Object> variables, UserProgram userprogram) {
		...
		for (int i = 0; i < totalADF; i++) {
			for (int j = 0; j < totalRPB; j++) {
				for (GrammarTerminalSymbol adf : grammars.get("RPB" + j)
						.getGrammarTerminalSymbols()) {
					if (adf.getSymbol().compareTo("ADF" + i) == 0) {
						((ADF) adf.getFunction()).setADFTree(trees.get("ADF"
								+ i));
					}
				}
			}
	 ...
	 *            de su ejecución.
	 * @param userProgram Referencia al programa modelado por el usuario.
...
//		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see progen.roles.Task#calculate()
	 */
	...
	public int compareTo(Object other) {
		return this.compareTo((Individual) other);
	}
	...
	public int compareTo(Individual other) {
		return (int) (rawFitness - other.rawFitness);
	}
	...
	public boolean equals(Individual other) {
		...
		if (other == null) {
			...
			for (int i = 0; i < totalRPB; i++) {
				equals = equals
						&& trees.get("RPB" + i).toString().compareTo(
								other.getTrees().get("RPB" + i).toString()) == 0;
			}
			for (int i = 0; i < totalADF; i++) {
				equals = equals
						&& trees.get("ADF" + i).toString().compareTo(
								other.getTrees().get("ADF" + i).toString()) == 0;
			}
		}
		return equals;
	}
