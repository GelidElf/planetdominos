...
public class HGPGrammar extends Grammar {
	...
	/**
	 * Constructor por defecto en el que se especifica el identificador del árbol, para 
	 * cargar la gramática correspondiente.
	 * @param idTree
	 */
	...
	/**
	 * Convierte una gramática normal, en una en la que todos los operadores están agrupados
	 * para generar formas de árbol, en función de su signatura.
	 */
	...
	private Function getWildCard(Production production, List<Production> productions){
		...
		for(Production pAux : productions){
			if(!pAux.equals(production)){ 							// si no son la misma produccion, se comparan todos los elementos
